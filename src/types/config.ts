...
export interface FileSelectorConfig {
  ...
  basePath?: string
  /**
   * Type of items that are valid selection options.
   *
  ...
  /**
   * If `true`, excluded files and directories (by the `filter` function) are shown in the list.
   * @default false
   */
  showExcluded?: boolean
  /**
   * Label displayed next to an item (file or directory) when it is disabled.
   * @default ' (not allowed)'
   */
